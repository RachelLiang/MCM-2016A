...
syms F0y F1y F2y F3y F4y F5y F6y l1 la a i Xml Yml delta;
g = 9.8;
l = 22.05;  %l1为虚拟锚链长度 
vf = 24;  %风速，手动输入控制
vs = 0;
...
if F6y <0
    ...
    la = la - 0.001;
    Xml = a*log((l-la+sqrt((l-la)^2+a^2))/a);
    i = lgg*(cos(theta1)+cos(theta2)+cos(theta3)+cos(theta4))+lgt*cos(theta5)+Xml+la;
end